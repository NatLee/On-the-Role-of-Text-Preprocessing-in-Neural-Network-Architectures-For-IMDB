...
class model():
    ...
    def print_log(self, id, predictions, X_test, y_test, i2w_map):
        with open('logs-pred/'+id+'.'+self.run_id+'.txt','w', encoding='utf-8') as ofile:
    ...
    def map2sentence(self, indexs, i2w_map):
        sen = []
        for ind in indexs:
            if i2w_map.get(ind) is not None:
                sen.append(i2w_map[ind])
        return " ".join(sen)


    def get_embeddings(self, path, data_w2index):
        #embed = open(path, 'r', encoding='iso8859-1')
        #done = 0
        #covered = dict()
        ...
        w2vModel = gensim.models.KeyedVectors.load_word2vec_format(path, binary=True)

        for i, word in enumerate(tqdm(data_w2index, ascii=True)):
            try:
                W[i] = w2vModel.get_vector(word)
            except KeyError:
                if missed.get(word) is None:
                    missed[word] = 1
                else:
                    missed[word] = missed[word] + 1

        
        print("Missed",len(missed.keys()),"words of", len(data_w2index))
    ...
    def classify(self, data, settings, log_id):
        ...
        print('Train...')
        model.fit(X_train, y_train, batch_size=settings['batch_size'], epochs=settings['nb_epoch'], callbacks=[checkpointer, earlystopper], validation_data=(X_test, y_test))
